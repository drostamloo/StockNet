...
def fetch_all_data():
    ...
    data = {k: v[column_titles].ewm(alpha=0.9, axis=0).mean().to_numpy()[-128:, :]
    ...
    index = index_data[column_titles].ewm(alpha=0.9, axis=0).mean()[-128:].to_numpy()
...
source = source_data[column_titles].ewm(alpha=0.9, axis=0).mean()[-128:].to_numpy()
...
source = np.divide(source_average - source_price_avg, source_price_std)
...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, d_timevec, num_heads, dff, pe_input, pe_target,
        ...
        #self.encoder3 = Encoder(num_layers, d_model, d_timevec, num_heads, dff, pe_input, rate)
        #self.encoder4 = Encoder(num_layers, d_model, d_timevec, num_heads, dff, pe_input, rate)
...
latest = tf.train.latest_checkpoint('{}_{}_{}_model/'.format(exchange, input_method, data_format))
transformer.load_weights(latest)


def evaluate(source, positive, negative, index):
    ...
    decoder_input = np.zeros((1, 1)).astype(np.float32)
    output = tf.expand_dims(decoder_input, 0)

    for i in range(64):
        look_ahead_mask = create_look_ahead_mask(output.shape[0])

        # predictions.shape == (batch_size (== 1), seq_len, num_features (== 5))
        predictions, attention_weights = transformer([source, index], output, False, look_ahead_mask)
...
def plot_line(source, predicted, true):
    date_range = pd.date_range(end=source_end, periods=128, freq='B').union(
        pd.date_range(start=pred_start, periods=64, freq='B'))
    ...
    ax = data.plot()
    fig = true.plot(ax=ax, title=company).get_figure()
    plt.axvline(source_end, linestyle='--', color='k')
    fig.savefig('figure.png')
    plt.draw()


def predict(source, positive, negative, index, true):
    result = evaluate(source, positive, negative, index)
    predicted = result[1:, :]
    true = true[1:, :]
    print(source)
    print(predicted)
    ...
    plot_line(source, predicted, true)


predict(dev_source[20, :, :], dev_positive[20, :, :], dev_negative[20, :, :], dev_index[20, :, :], dev_target[20, :, :])
plt.show()
