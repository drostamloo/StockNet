...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, d_timevec, num_heads, dff, maximum_position_encoding, rate=0.1):
        ...
        self.dec_layers = [SingleDecoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
        #self.dropout = tf.keras.layers.Dropout(rate)
...
checkpoint_path = 'gs://tpu_daniel/{}_{}0_{}_checkpoints'.format(exchange, input_method, data_format)
...
train_step_signature = [
    ...
    tf.TensorSpec(shape=(128, 65, 1), dtype=tf.float32)
]
...
with strategy.scope():
    ...
    def dev_step(source, target):
        output = tf.zeros((tf.shape(target)[0], 1, 1))

        for i in range(target.get_shape()[1] - 1):
            tf.autograph.experimental.set_loop_options(shape_invariants=[output, tf.TensorShape([target.get_shape()[0], None, 1])])
...
transformer.save_weights('gs://tpu_daniel/{}_{}0_{}_model/{}_{}_{}'.format(exchange, input_method, data_format, exchange, input_method, data_format))
