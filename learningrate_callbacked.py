...
class PositionalEncoding(layers.Layer):
    ...
    def positional_encoding(self, position, timevec_dim):
...
def noam_decay(global_step, embed_dim, warmup_steps):
    arg1 = 1 / np.sqrt(global_step, dtype=np.float32)
    arg2 = np.multiply(global_step, np.power(warmup_steps, -1.5, dtype=np.float32), dtype=np.float32)

    return (1 / np.sqrt(embed_dim, dtype=np.float32)) * min(arg1, arg2)


class NoamDecayScheduler(keras.callbacks.Callback):
    def __init__(self, embed_dim, warmup_steps=4000, global_step_init=1, verbose=0):
    ...
    def on_batch_begin(self, batch, logs=None):
        lr = noam_decay(self.global_step, self.embed_dim, self.warmup_steps)
...
with strategy.scope():
    noam_lr_scheduler = NoamDecayScheduler(embed_dim)
...
model.fit(train_data, epochs=EPOCHS, callbacks=[noam_lr_scheduler, checkpoint_callback], validation_data=dev_data)
