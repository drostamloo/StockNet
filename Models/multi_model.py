...
class SingleDecoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, rate=0.1):
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, d_timevec, num_heads, dff, maximum_position_encoding, rate=0.1):
        ...
        self.dec_layers = [SerialDecoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
        #self.dropout = tf.keras.layers.Dropout(rate)
    ...
    def call(self, x, sources, training, look_ahead_mask):
        ...
        for i in range(self.num_layers):
            ...
            #attention_weights['decoder_layer{}_block6'.format(i+1)] = blocks[5]
...
with strategy.scope():
    ...
    train_MAE_accuracy = tf.keras.metrics.MeanAbsoluteError(name='train_MAE_accuracy')
    train_MSE_accuracy = tf.keras.metrics.MeanSquaredError(name='train_MSE_accuracy')

    dev_MAE_accuracy = tf.keras.metrics.MeanAbsoluteError(name='dev_MAE_accuracy')
    dev_MSE_accuracy = tf.keras.metrics.MeanSquaredError(name='dev_MSE_accuracy')

    test_MAE_accuracy = tf.keras.metrics.MeanAbsoluteError(name='test_MAE_accuracy')
    test_MSE_accuracy = tf.keras.metrics.MeanSquaredError(name='test_MSE_accuracy')
...
with strategy.scope():
    ...
    def train_step(source, positive, target):
        tar_inp = target[:, :-1, :]
        tar_real = target[:, 1:, :]
        ...
        with tf.GradientTape() as tape:
            predictions, _ = transformer([source, positive], tar_inp, True, look_ahead_mask)
        ...
        train_loss(loss)
        train_MAE_accuracy.update_state(tar_real, predictions)
        train_MSE_accuracy.update_state(tar_real, predictions)


    @tf.function
    def distributed_train_step(source, positive, target):
        strategy.run(train_step, args=(source, positive, target))


    @tf.function(input_signature=train_step_signature)
    def dev_step(source, positive, target):
        tar_inp = target[:, :-1, :]
        tar_real = target[:, 1:, :]

        look_ahead_mask = create_look_ahead_mask(tf.shape(tar_inp)[1])

        predictions, _ = transformer([source, positive], tar_inp, False, look_ahead_mask)

        dev_MAE_accuracy.update_state(tar_real, predictions)
        dev_MSE_accuracy.update_state(tar_real, predictions)


    @tf.function
    def distributed_dev_step(source, positive, target):
        strategy.run(dev_step, args=(source, positive, target))


    @tf.function(input_signature=train_step_signature)
    def test_step(source, index, target):
        tar_inp = target[:, :-1, :]
        tar_real = target[:, 1:, :]

        look_ahead_mask = create_look_ahead_mask(tf.shape(tar_inp)[1])

        predictions, _ = transformer([source, index], tar_inp, False, look_ahead_mask)

        test_MAE_accuracy.update_state(tar_real, predictions)
        test_MSE_accuracy.update_state(tar_real, predictions)


    @tf.function
    def distributed_test_step(source, positive, target):
        strategy.run(test_step, args=(source, positive, target))

for epoch in range(EPOCHS):
    start = time.time()

    train_loss.reset_states()
    train_MAE_accuracy.reset_states()
    train_MSE_accuracy.reset_states()

    # inp -> history, positives, negatives, index; tar -> target
    for batch, (source, positive, negative, index, target) in enumerate(train_data):
        distributed_train_step(source, positive, target)

        if batch % 50 == 0:
            print('Epoch {} Batch {} Loss {:.4f} MAE {:.4g} MSE{:.4g}'.format(epoch + 1,
                                                                              batch,
                                                                              train_loss.result(),
                                                                              train_MAE_accuracy.result(),
                                                                              train_MSE_accuracy.result()))

    print('Epoch {} Loss {:.4f} MAE {:.4g} MSE{:.4g}'.format(epoch + 1,
                                                             train_loss.result(),
                                                             train_MAE_accuracy.result(),
                                                             train_MSE_accuracy.result()))
    ...
    if (epoch + 1) % 5 == 0:
        for batch, (source, positive, negative, index, target) in enumerate(dev_data):
            distributed_dev_step(source, positive, target)

        print('Validation: Epoch {} MAE {:.4g} MSE{:.4g}\n'.format(epoch + 1,
                                                                   dev_MAE_accuracy.result(),
                                                                   dev_MSE_accuracy.result()))
        dev_MAE_accuracy.reset_states()
        dev_MSE_accuracy.reset_states()

for batch, (source, positive, negative, index, target) in enumerate(test_data):
    distributed_test_step(source, positive, target)

print('\nTest performance: MAE {:.4g} MSE {:.4g}'.format(test_MAE_accuracy.result(),
                                                         test_MSE_accuracy.result()))
